...
import io.protoforce.runtime.transport.dispatch.server.Envelopes.AsyncResponse
...
object PollingConfig {
  ...
  val default: PollingConfig = PollingConfig(100.milliseconds, 20)
}
...
object Envelopes {
  ...
  object AsyncRequest {
    ...
    private implicit def ServiceName_codec: Codec[ServiceName] = Codec.from(Decoder.decodeString.map(ServiceName.parse), Encoder.encodeString.contramap(_.toString))
  ...
  object AsyncResponse {
    ...
    implicit def EnvelopePoly_codec: Codec[AsyncResponse] = Codec.from(
  ...
  object RemoteError {
    ...
    case class ShortException(kind: String, message: Option[String])
    object ShortException {
      def of(t: Throwable): ShortException = ShortException(t.getClass.getName, Option(t.getMessage))
